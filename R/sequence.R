...
exe_gblocks <- function(fasta_path, gblocks_path = "/Users/hu_zhehao/Desktop/Biology-tools/Gblocks_0.91b/Gblocks", t="d", b1="min", b2="min", b3=20, b4=2, b5="a", args=""){
    ...
    if (length(gbFile) > 1) {
        cli_abort("Multiple fasta-gb files.")
    } else if (length(gbFile) == 0) {
        cli_abort("No fasta-gb files.")
    }
...
#' Plot out distance histogram
#' @param distmat_path Path to distmat txt file generated by ABGD program
#' 
#' @seealso [abgd_excute()]
abgd_disthist <- function(distmat_path) {
    distmat_raw <- read.table(distmat_path, sep = " ", skip = 1)
    distmat.plot <- distmat_raw[,-1] %>% 
...
concatenate_alignment <- function(
    ...
    res <- list(
        ...
        sequence_composition = concat.seq.comp %>% filter(c_organism_label %in% dimnames(concat.alignment)[[1]])
    )
}
